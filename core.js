...
export function createMCPHandler({ fetchFn, cache, logger }) {
  async function handle(message) {
    ...
    switch (message.method) {
      case 'server/discover':
        return success(id, { serverInfo: SERVER_INFO, protocolVersions: [MCP_VERSION], capabilities: { tools: { listChanged: false } } });
      case 'tools/list':
  ...
  const toolGetDCXIndex = { name: 'get_dcx_index', title: 'Get DCX Index', description: 'Return the OpenFacet Diamond Composite Index with 24-hour, 7-day, and 30-day trends.', inputSchema: { type: 'object', properties: {}, additionalProperties: false } };
  const toolGetMarketDepth = { name: 'get_market_depth', title: 'Get Market Depth', description: 'Return comparable observed-offer depth, optionally at a carat weight.', inputSchema: { type: 'object', properties: { carat: { type: 'number', minimum: 0.3, maximum: 6 } }, additionalProperties: false } };
  return handle;
}
